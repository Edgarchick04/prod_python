...
from handlers.commands import in_walk_handler
from handlers.keyboards import TaskKeyboard
from states.walk_state import WalkState


@pytest.mark.asyncio
async def test_in_walk_generate_task(storage, storage_key):
    message = AsyncMock()
    ...
    await state.update_data(task_state="no_task")
    task = {"description": "any task"}
    with patch("handlers.commands.task_generator", AsyncMock(return_value=task)) as mock_task_generator:
...
async def test_in_walk_finish_without_task(storage, storage_key):
    ...
    with patch("handlers.commands.finish_walk", AsyncMock()) as mock_finish_walk:
        await in_walk_handler(message, state)
...
@pytest.mark.asyncio
async def test_in_walk_generate_another_task(storage, storage_key):
    message = AsyncMock()
