...
from handlers.commands import statistics_handler
...
async def test_statistics_get_stats(storage, storage_key, user_id):
    ...
    with patch("handlers.commands.get_stats", AsyncMock(return_value=stats)) as mock_get_stats:
...
async def test_statistics_get_walks_data(storage, storage_key, user_id):
    ...
    await state.set_state(StartState.user_walks)
    with patch("handlers.commands.get_walks_data", AsyncMock(return_value=walks_data)) as mock_get_walks_data:
        await statistics_handler(message, state)
