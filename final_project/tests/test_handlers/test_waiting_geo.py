...
from handlers.commands import waiting_geo_handler
from handlers.keyboards import WalkKeyboard
from states.walk_state import WalkState


@pytest.mark.asyncio
async def test_waiting_geo_sent_geo(storage, storage_key, latitude, longtitude):
    message = AsyncMock()
    ...
    with patch("handlers.commands.route_generator", AsyncMock(return_value=route)):
